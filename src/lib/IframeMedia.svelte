...
class="iframe-container {cssClass} --i: {i}" data-ready={ready} style="position: relative; --aspect-ratio: {aspectRatio}; {mediaItem?.poster?`background-image:url(${`/projects/${mediaItem.project.year}/${mediaItem.project.slug}/media/thumb512/${mediaItem.poster.hash}.webp`});`:''}"
